...
class LeetCodeStateManager {
  ...
  broadcastState() {
    chrome.runtime.sendMessage(
      { type: "statsUpdate", data: this.getStats() },
      () => {
        if (chrome.runtime.lastError) {
          console.warn("No receiver for statsUpdate message.");
        }
      }
    );
  }
...
class GitHubService {
  ...
  async buildBasicGithubUrl() {
    ...
    if (!result.leetcode_scrapper_username || !result.leetcode_scrapper_repo) {
      throw new Error("GitHub username or repository name is missing.");
    }
    return `${this.env.REPOSITORY_URL}${result.leetcode_scrapper_username}/${result.leetcode_scrapper_repo}/contents/`;
  }
  ...
  async getAllLeetCodeProblems() {
    try {
      ...
      console.error("Failed to fetch GitHub problems:", error);
      return [];
    }
  }
...
class LeetCodescrapperController {
  constructor() {
  ...
  saveUserInfos(request) {
    chrome.storage.local.set({
      leetcode_scrapper_username: request.username,
      leetcode_scrapper_token: request.token,
    });
  }
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install" || details.reason === "update") {
    chrome.storage.local.get("leetcode_scrapper_code_submit", (result) => {
      if (result.leetcode_scrapper_code_submit === undefined) {
        chrome.storage.local.set({ leetcode_scrapper_code_submit: true });
      }
    });
  }
});
...
// Initialization
const controller = new LeetCodescrapperController();

...
class Github {
  ...
  async submitToGitHub(dataConfig, userConfig) {
    ...
    if (fileExists) {
      ...
      const result = await chrome.storage.local.get(
        "leetcode_scrapper_code_submit"
      );
      const skipDuplicates = result.leetcode_scrapper_code_submit;
  ...
  buildGitHubUrl(dataConfig, userConfig, file = "") {
    ...
    return `${dataConfig.REPOSITORY_URL}${userConfig.leetcode_scrapper_username}/${userConfig.leetcode_scrapper_repo}/contents/${this.problem.slug}/${fileName}`;
  }
  ...
  async fetchWithAuth(url, method, dataConfig, userConfig, body = null) {
    const options = {
      ...
      headers: {
        ...dataConfig.HEADERS,
        Authorization: `token ${userConfig.leetcode_scrapper_token}`,
      },
    };
...
class Leetcodescrapper {
  ...
  isConfigValid(config) {
    return (
      config.leetcode_scrapper_repo &&
      config.leetcode_scrapper_username &&
      config.leetcode_scrapper_token
    );
  }
...
var scrapper = new Leetcodescrapper();
scrapper.init();
